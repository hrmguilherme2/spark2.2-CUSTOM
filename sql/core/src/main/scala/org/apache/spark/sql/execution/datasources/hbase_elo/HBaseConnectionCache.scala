 ...
 */
package org.apache.spark.sql.execution.datasources.hbase_elo
...
private[spark] object HBaseConnectionCache extends Logging {
  ...
  private[hbase_elo] val connectionMap = new mutable.HashMap[HBaseConnectionKey, SmartConnection]()
  ...
  // 'duration' specified
  private[hbase_elo] def sleep(duration: Long, allowInterrupt: Boolean = false, allowClosed: Boolean = false): Unit = {
  ...
  private[hbase_elo] def resetCache(resetStats: Boolean = false): Unit = {
    connectionMap.synchronized {
      if (closed.get()) return
  ...
  private[hbase_elo] def performHousekeeping(forceClean: Boolean) = {
...
private[hbase_elo] class SmartConnection(
...
private[hbase_elo] object HBaseConnectionKey {
...
class HBaseConnectionCacheStat(private var _numTotalRequests: Long,
  ...
  private[hbase_elo] def incrementActualConnectionsCreated(incr: Long) = {
    _numActualConnectionsCreated += incr
  }

  private[hbase_elo] def incrementTotalRequests(incr: Long) = {
    _numTotalRequests += incr
  }

  private[hbase_elo] def setActiveConnections(numActiveConnections: Long) = {
    this._numActiveConnections = numActiveConnections
  }

  private[hbase_elo] def copy(): HBaseConnectionCacheStat =
  ...
  private[hbase_elo] def reset(): Unit = {
    _numTotalRequests = 0
