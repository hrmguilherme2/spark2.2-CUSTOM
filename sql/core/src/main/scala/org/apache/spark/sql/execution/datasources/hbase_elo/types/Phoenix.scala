...
package org.apache.spark.sql.execution.datasources.hbase_elo.types
...
import org.apache.spark.sql.execution.datasources.hbase_elo._
...
class Phoenix(f: Option[Field] = None) extends SHCDataType {
  ...
  override def encodeCompositeRowKey(rkFields: Seq[(Int, Field)], row: Row): Seq[Array[Byte]] = {
    rkFields.map { case (x, y) =>
      var ret = toBytes(row(x))
      // the last dimension of composite key does not need SEPARATOR
      if (y.length == -1 && x < rkFields.size - 1) {
